...
class Media {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  bend: number; textColor: string; borderRadius: number; font: string; program: any; plane: any; title: any;
  speed = 0; width = 0; widthTotal = 0; x = 0; padding = 0; isBefore = false; isAfter = false;
...
class App {
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    if (Math.abs(x - this.clickStartX) > 5) this.isClick = false;
  }
  ...
  handleClick(clientX: number, clientY: number) {
    if (!this.gl?.canvas || !this.medias || !this.onClickCallback) return;
    const canvas = this.gl.canvas as HTMLCanvasElement;
    const rect = canvas.getBoundingClientRect();
    if (clientX < rect.left || clientX > rect.right || clientY < rect.top || clientY > rect.bottom) return;
    const x = clientX - rect.left;
    const viewX = ((x / rect.width) * 2 - 1) * (this.viewport.width / 2);
    let clicked: { image: string; text: string } | null = null;
    let minAbsX = Infinity;
    for (const media of this.medias) {
      const px = media.plane.position.x;
      const halfW = media.plane.scale.x / 2;
      if (Math.abs(px - viewX) <= halfW) {
        const absX = Math.abs(px - viewX);
        if (absX < minAbsX) { minAbsX = absX; clicked = { image: media.image, text: media.text }; }
      }
    }
    if (clicked) this.onClickCallback(clicked);
  }
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const app = new App(el, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase });
    app.onClickCallback = setLightboxItem;
    return () => app.destroy();
    ...
    <>
      ...
      {lightboxItem && (
        ...
        >
          ...
          <div
            className="relative flex flex-col items-center gap-3 max-w-[90vw] max-h-[90vh] px-4"
            onClick={(e) => e.stopPropagation()}
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={lightboxItem.image}
              alt={lightboxItem.text}
              className="max-w-full max-h-[82vh] object-contain rounded-xl shadow-2xl"
            />
            {lightboxItem.text && (
              <p className="text-white/60 text-sm text-center">{lightboxItem.text}</p>
            )}
          </div>
        </div>
      )}
